...
int main(int argc, char *argv[])
{
    ...
    if (local_server <1)
    {
        printf("Error connecting to local server. Exiting...\n");
        exit(-1);
    }
    ...
    while (1)
    {
        ...
        {
            printf("You have to establish a connection first using connect. Try again \n");
            continue;
        }
        if (n_read < 3)
        {
    ...
    while (1)
    {
        ...
        if (strcmp(command, "delete") == 0)
        {
            ...
            if (status_app == 1 || status_app == -2) continue;
            if (status_app == -1 || status_app == -5) exit(-1);
        }
        if (strcmp(command, "connect") == 0)
        {
            printf("Already connected to a group.\n");
            continue;
        }
        else
        {
